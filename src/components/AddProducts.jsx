...
const AddProducts = () => {
  const [productDetails, setProductDetails] = useState({
    productCode: "", name: "", price: "", available_country: "", tax_rate: "", color: "", size: "", stock: "", category: "", offer: "", description: "", brand: "", ratings: "", productImages: [], // For files
  });
  ...
  const handleOpenAddModalOpen = () => setShowAddModal(true)
  const handleAddModalClose = () => setShowAddModal(false)

  const getAllProducts = async () => {
    try {
      const response = await getAllProductAPI()
      console.log(response.data.allProductsList);
      if (response.status === 200) {
        setAllProducts(response.data.allProductsList)
      }
  ...
  const handleEditClick = (product) => {
    setEditProductDetails({
      ...
      name: product.name || "",
      price: product.price || "",
      available_country: product.available_country || "",
    ...
    setEditPreviewUrls(
      product.productImages && product.productImages.length > 0
        ? product.productImages.map((img) => `${SERVER_URL}/${img}`)
        : [null, null, null]
    );
  ...
  // 🔹 Handle input/image change for edit modal
  ...
  const handleUpdateProduct = async () => {
    ...
    if (editProductDetails.productImages) {
  ...
  try {
    const response = await deleteProductAPI(selectedProduct._id);

    if (response.status === 200) {
      alert("Product deleted successfully!");
      
      // Remove from local state / context
      setProducts((prev) =>
  ...
  return (
    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        {[
          { label: "Total Products", value: allProducts.length, color: "#c8a876" },
          { label: "Revenue", value: "$24,500", color: "#22c55e" },
          { label: "New Orders", value: "320", color: "#3b82f6" },
        ].map((card, i) => (
          <div
            key={i}
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
        <div className="p-4 border-b flex justify-between items-center">
          ...
          <button
            onClick={handleOpenAddModalOpen}
            className="bg-[#c8a876] text-white py-2 px-5 rounded-lg shadow hover:bg-[#d6a455] transition"
          >
            + Add Product
          </button>
        </div>
                  ...
                  >
                    <td className="p-4">
                      <div className="flex gap-2 flex-wrap">
                        {prdt.productImages && prdt.productImages.length > 0 ? (
                          prdt.productImages.slice(0, 3).map((img, index) => (
                            <img
                              key={index}
                              src={`${SERVER_URL}/${img}`}
                              alt={`Product ${index + 1}`}
                              className="w-40 h-40 object-contain p-1 rounded-md shadow-sm hover:scale-105 transition-transform"
                            />
                          ))
                        ) : (
                    ...
                    <td className="p-4 text-sm leading-snug">
                      ...
                      <div className="mt-3">
                        <button onClick={handleEditClick} className="px-3 py-1 bg-blue-500 text-white rounded-md text-xs hover:bg-blue-600 transition">
                          Edit
                        </button>
                        <button onClick={() => {
                          setSelectedProduct(prdt)
                          setShowDeleteModal(true)
                        }} className="px-3 py-1 bg-red-500 text-white rounded-md text-xs ms-2 hover:bg-red-600 transition">
                          Delete
                        </button>
                      </div>
                    </td>
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50">
          <div className="bg-white rounded-2xl shadow-lg w-[90%] md:w-[70%] max-h-[90vh] overflow-y-auto transition-all duration-300">
              ...
              <form className="space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {[0, 1, 2].map((index) => (
                    <label
                      key={index}
                      className="border-2 border-dashed border-gray-300 rounded-xl flex flex-col items-center justify-center p-4 cursor-pointer hover:bg-gray-50 transition"
                    >
                      <input
                        type="file"
                        className="hidden"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {[
                    ...
                    { label: "Available Country", name: "available_country" },
                    ...
                    <div key={name} className="flex flex-col">
                      ...
                      <input
                        ...
                        className="border rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-[#c8a876]"
                  ...
                  <div className="col-span-2">
                    ...
                    <textarea
                      ...
                      onChange={handleInputChange}
                      className="border rounded-lg p-2 w-full focus:outline-none focus:ring-2 focus:ring-[#c8a876]"
                      rows="3"
            ...
            <div className="p-6 border-t flex justify-end gap-3">
              <button
                ...
                className="bg-[#c8a876] text-white px-5 py-2 rounded-lg hover:bg-[#d6a455] transition"
              >
                Save Product
              </button>
              <button
        ...
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50">
          <div className="bg-white rounded-2xl shadow-lg w-[90%] md:w-[70%] max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {[0, 1, 2].map((index) => (
                  <label key={index} className="border-2 border-dashed border-gray-300 rounded-xl flex flex-col items-center justify-center p-4 cursor-pointer hover:bg-gray-50">
                    ...
                    {editPreviewUrls[index] ? (
                      <img src={editPreviewUrls[index]} alt={`Preview ${index + 1}`} className="w-full h-36 object-contain rounded-lg" />
                    ) : (
                      <p className="text-gray-500 text-sm">Click to upload Image {index + 1}</p>
                    )}
                  </label>
                ))}
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  "name", "price", "available_country", "tax_rate", "color",
                  "size", "stock", "category", "offer", "brand", "ratings"
                ].map((name) => (
                  <div key={name}>
                    <label className="text-sm text-gray-600 mb-1 capitalize">{name}</label>
                    <input
                      type="text"
                      name={name}
